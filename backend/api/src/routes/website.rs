...
#[handler]
pub fn get_website(Path(name): Path<String> , Data(s) : Data<&Arc<Mutex<Store>>>) -> String {
    let mut s = s.lock().unwrap();
    let web = s.get_website(name).unwrap();
    return format!("Hello, {}", web.id);
} 
#[handler]
pub fn create_website(Json(data) : Json<CreateWebsiteRequest>, Data(s) : Data<&Arc<Mutex<Store>>>) -> Json<CreateWebsiteResponse> {
